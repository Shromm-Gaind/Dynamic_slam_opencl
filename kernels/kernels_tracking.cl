...
__kernel void se3_LK_grad(
	...
	__local		float4*	local_sum_weight,		//13	1 DoF, float4 channels
	__global 	float4*	global_sum_weight,		//14

	__global 	float4*	SE3_incr_map_,			//15
	__local		float4*	local_sum_grads,		//16
	__global	float4*	global_sum_grads		//17
	)
 {																									// find gradient wrt SE3 find global sum for each of the 6 DoF
	uint  global_id_u 	= get_global_id(0);
	float global_id_flt = global_id_u;
...
*/
	...
	if (  intersection  ) {																			// if (not cleanly within new frame) skip  Problem u2&v2 are wrong.
		...
		weight = bilinear_grad_weight(HSV_grad, read_index, u2_flt/reduction, v2_flt/reduction, mm_cols, read_offset_, reduction);
